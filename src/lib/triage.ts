...
每篇文章只提取 1-2 个最核心的概念，宁少勿滥。

### 第二步：溯源挖掘
对每个提取出的概念，**必须用 WebSearch 和 WebFetch** 去追根溯源，找到一手来源（官方仓库、论文、技术文档）。
...
===TRIAGE_START===
{
  ...
  "concepts": [
    {
      "name": "概念名",
      "root": "起源（谁/哪篇论文/哪个项目）→ 核心机制 → 突破点（3-5句话）",
      "whatItEnables": "拿到它能做什么、造什么（具体场景）",
      "sourceUrl": "你通过工具查到的一手来源 URL（官方仓库/论文/文档）"
    }
  ],
...
- root 要写清楚三层：起源 → 核心机制 → 突破点，不是名词解释
- **每个 concept 的 sourceUrl 必须是你通过工具查到的一手来源（官方仓库/论文/文档），不是原文链接。sourceUrl 为空的 concept 视为无效。**
...
研究完成后，按系统提示要求输出 JSON。每个 concept 必须附带你通过工具查到的 sourceUrl，没有 sourceUrl 的 concept 视为无效。`
      : `请研究这个链接讨论的技术：
...
这是二手推文的抓取内容，不要只看这篇文章就下结论。请用 WebSearch/WebFetch 找到每个概念的一手来源（官方仓库、论文、文档等），每个 concept 必须附带你查到的 sourceUrl，没有 sourceUrl 的 concept 视为无效。`;
