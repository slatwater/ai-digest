...
import { ChatMessage, TriageModel, resolveModelId, ExperimentExecutor } from './types';
...
interface ExperimentSession {
  ...
  model: string;
  executors: ExperimentExecutor[]; // 用户勾选的执行者，决定 system prompt 的工具/平台指引
...
// 根据用户勾选的执行者拼装"执行手段"附录
// - 含 coze：嵌入 COZE_CLI_GUIDE，允许调云平台
// - 含 claude：嵌入"本地直跑"指引，允许 agent 在 workDir 里自己写脚本/prompt 跑
// - 两个都选：并存，提示 agent 按场景挑或并用；仅一个则禁用另一个
function buildExecutorAppendix(executors: ExperimentExecutor[]): string {
  const hasCoze = executors.includes('coze');
  const hasClaude = executors.includes('claude');

  const claudeBlock = `# Claude 本地执行手册（agent 自跑）

不依赖外部平台，agent 直接在工作目录里完成验证：
- **写最小验证脚本**：用 \`Write\` 写 .py / .ts / .sh 等脚本到工作目录；用 \`Bash\` 跑（python3、node、bash 等），观察 stdout/stderr。
- **写 prompt 草稿**：把要验证的 prompt 存为 \`prompt.md\`，再用 \`Bash\` 调本机已装的模型/工具（如 \`curl\` 调 OpenAI/Anthropic API、本地 Ollama、其它 CLI）。Anthropic / OpenAI key 走环境变量。
- **WebFetch 拉一手数据**：抓样例文档/API 文档作为脚本输入。
- **不要做的事**：不要 \`pip install\` / \`npm install\` 大型依赖（沙盒会被清理）；不要起长驻服务；不要写超过 ~200 行的脚本。
- **结果直接对话里给**：脚本运行结果（stdout、关键截取）总结进对话消息，不要让用户去读临时文件。
`;

  if (hasCoze && hasClaude) {
    return `# 执行手段（你有两条路径可选，按场景挑或并用）

**怎么选**：
- 需要真实跑 agent / workflow / app / 多模态生成 / 长流水线 → 走 **Coze 平台**（cli 在下方）
- 只是想验证一段 prompt、跑个最小脚本、对比几次模型输出 → 走 **Claude 本地直跑**（在 workDir 里写脚本即可）
- 想做对照（同一 prompt 在两边各跑一次） → 两个都用，结果分别贴回对话
...
**本会话已禁用 Coze 云平台**——所有验证在工作目录沙盒里用本机工具完成，不要调 \`coze\` CLI。

${claudeBlock}`;
  }
  // 默认 / 仅 coze
  return COZE_CLI_GUIDE;
}

// 构建 system prompt：研究员角色
function buildExperimentPrompt(session: ExperimentSession): string {
  const executors = session.executors.length ? session.executors : (['coze'] as ExperimentExecutor[]);
  const executorLabels = executors
    .map(x => x === 'coze' ? 'Coze 云平台（coze CLI）' : 'Claude 本地直跑（Bash/Read/Write/WebFetch）')
    .join(' + ');
...
5. **执行验证**：见下方「执行手段」章节，按你被授权的执行者操作；未被授权的执行者不要调用。
...
${buildExecutorAppendix(executors)}`;
  }
  ...
  return `你是一个「实验研究员」。职责是：基于用户选中的 Wiki 条目的一手链接做调研，与用户对话讨论方案，按授权的执行者做小规模验证，最终沉淀可复用的"经验"。
...
4. **执行验证**：见下方「执行手段」章节，按你被授权的执行者操作；未被授权的执行者不要调用。
...
function normalizeExecutors(raw: unknown): ExperimentExecutor[] {
  if (!Array.isArray(raw)) return ['coze']; // 兼容老调用
  const out: ExperimentExecutor[] = [];
  for (const v of raw) {
    if ((v === 'coze' || v === 'claude') && !out.includes(v)) out.push(v);
  }
  return out.length ? out : ['coze'];
}
...
export async function runExperiment(
  ...
  const session = await getOrCreateSession(sessionId, source, model, normalizeExecutors(executors));
