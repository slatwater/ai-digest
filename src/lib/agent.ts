...
import { compileWiki } from './compiler';
...
`.trim();
  ...
  const demoPrompt = `你是一个前端 Demo 生成专家。基于以下研究内容，生成一个可交互的浏览器 Demo。不要读取任何文件，不要执行任何命令，直接输出。只输出 ===DEMO_START=== 和 ===DEMO_END=== 包裹的 JSON，不要输出其他内容。所有输出用中文。
...
===DEMO_END===`;
  ...
  try {
    ...
    if (demoMatch) {
      return safeParseJSON<{ language: string; filename: string; code: string; instructions: string }>(demoMatch[1], 'generateDemo');
    }
...
export async function runDigest(
  ...
  // 阶段顺序，只允许单向前进（practice 由独立 Agent 处理，不在主流程中）
  const phaseOrder: DigestPhase[] = ['capture', 'trace', 'analyze', 'archive'];
  ...
  try {
    ...
    // Wiki 编译（异步，不阻塞主流程）
    compileWiki(entry).catch(err => {
      console.warn('[compiler] Wiki 编译失败:', err);
    });
